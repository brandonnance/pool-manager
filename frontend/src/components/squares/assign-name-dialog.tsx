...
export function AssignNameDialog({
  ...
  const [allSquaresData, setAllSquaresData] = useState<
    Array<{ id: string; participant_name: string | null; verified: boolean }>
  >([]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-4">
          ...
          {isAssigned &&
            participantStats &&
            participantStats.totalSquares > 1 && (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    <span className="text-green-600">
                      {participantStats.verifiedCount}
                    </span>
                    {" / "}
                    <span className="text-red-600">
                      {participantStats.unverifiedCount}
                    </span>{" "}
                    verified/unverified
                  </span>
                </div>
