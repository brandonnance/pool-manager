...
import { LinkTeamsButton } from './link-teams-button'
...
export function MarchMadnessContent({
  ...
  const drawDone = mmPool.draw_completed
  const teamsLinked = mmPool.teams_linked

  // Pool is fully set up only when draw is done AND teams are linked
  const needsSetup = !drawDone || !teamsLinked

  if (needsSetup && isCommissioner) {
    // Determine which flow we're in based on current state
    const isPreDrawDone = drawDone && !teamsLinked // positions assigned, awaiting teams
    const hasPartialTeams = poolTeams.length > 0 && poolTeams.length < 64
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            ...
            {/* Step: Run Draw (position or traditional) */}
            {(() => { stepNum++; return null })()}
            <div className="flex items-center gap-3">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${
                drawDone ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'
              }`}>
                {drawDone ? '✓' : stepNum}
              </div>
              <div className="flex-1">
                <p className="font-medium">
                  {teamsReady && !drawDone ? 'Run Random Draw' : drawDone ? 'Position Draw' : 'Run Position Draw'}
                </p>
                <p className="text-sm text-muted-foreground">
                  {drawDone
                    ? `Completed${mmPool.draw_completed_at ? ` ${new Date(mmPool.draw_completed_at).toLocaleDateString()}` : ''}`
                    : teamsReady
                      ? 'Assign teams to entries randomly'
                      : 'Assign bracket positions (region + seed) before teams are announced'}
                </p>
              </div>
              {entriesReady && !drawDone && (
                <RandomDrawButton
                  mmPoolId={mmPool.id}
            ...
            {/* Step: Add Teams (only shown if draw done without teams, or teams not yet ready) */}
            {(isPreDrawDone || (!drawDone && !teamsReady)) && (
              <>
                {(() => { stepNum++; return null })()}
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${
            ...
            {/* Step: Link Teams (only in pre-draw flow) */}
            {isPreDrawDone && (
              <>
                {(() => { stepNum++; return null })()}
                <div className="flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${
                    teamsLinked ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'
                  }`}>
                    {teamsLinked ? '✓' : stepNum}
                  </div>
                  <div className="flex-1">
                    <p className="font-medium">Link Teams to Entries</p>
                    <p className="text-sm text-muted-foreground">
                      {teamsLinked ? 'Completed' : 'Match loaded teams to position assignments'}
                    </p>
                  </div>
                  {teamsReady && !teamsLinked && (
                    <LinkTeamsButton
                      mmPoolId={mmPool.id}
                      teamCount={poolTeams.length}
          ...
          {/* Show position assignments after pre-draw */}
          {isPreDrawDone && (
            <div className="mt-6">
              <TeamDrawDisplay
                entries={entries}
  ...
  if (needsSetup && !isCommissioner) {
    // Non-commissioner view during setup
    const isPreDrawDone = drawDone && !teamsLinked
    return (
      <div className="space-y-6">
        <Card>
          <CardContent className="py-8 text-center">
            <h3 className="text-lg font-semibold mb-2">
              {isPreDrawDone ? 'Position Draw Complete' : 'Pool Setup in Progress'}
            </h3>
            <p className="text-muted-foreground mb-4">
              {isPreDrawDone
                ? 'Positions have been drawn. Waiting for teams to be announced and linked.'
            ...
            <div className="flex justify-center gap-4">
              ...
              {drawDone && (
                <Badge variant="default">Positions Drawn</Badge>
              )}
            </div>
          </CardContent>
        </Card>

        {/* Show position assignments to non-commissioners too */}
        {isPreDrawDone && (
          <TeamDrawDisplay
            entries={entries}
            poolTeams={poolTeams}
